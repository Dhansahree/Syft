...
class BlogRepositoryTest {
    ...
    fun `get posts returns cached values if available`() {
        ...
        val observer = sut.getPosts(1, 20).test()
        observer.assertValue(listOf(anyPost))
        verify(exactly = 0) { blogApi.getPosts(1, 20) }
    }

    @Test
    fun `posts value fetched from api is inserted to the cache`() {
        every { postDao.getAll() } returns Single.just(listOf())
        every { blogApi.getPosts(1, 20) } returns Single.just(listOf(anyPost))

        sut.getPosts(1, 20).test()

        verify {
            blogApi.getPosts(1, 20)
    ...
    fun `value from api is returned to caller`() {
        ...
        val postObserver = sut.getPosts(1, 20).test()
    ...
    fun `api failing returns reactive error on chain`() {
        every { postDao.getAll() } returns Single.just(listOf())
        val error = Throwable()
        every { blogApi.getPosts(1, 20) } throws error
    ...
    @Test
    fun `delete post returns value completion signal on success`() {
        every { postDao.delete(1) } returns Completable.complete()

        val observer = sut.deletePost(1).test()
        observer.assertComplete()
    }

    @Test
    fun `delete post returns value error signal on fail`() {
        every { postDao.delete(1) } returns Completable.error(Throwable())
